...
/**
 * The {@code SchedulingApp} class provides a graphical user interface for
 * managing and scheduling tasks.
 * 
 * <p>
 * This application displays a calendar, allowing the user to:
 * <ul>
 * <li>View tasks scheduled on each day of a selected month and year.</li>
 * <li>Create new tasks for specific dates using an {@link AddTaskDialog}.</li>
 * <li>View all tasks for a particular date using a popup menu.</li>
 * </ul>
 * 
 * <p>
 * Cells in the calendar are color-coded:
 * <ul>
 * <li>Days with tasks are highlighted in a distinct color.</li>
 * <li>Days outside of the currently displayed month are grayed out.</li>
 * <li>Hovering over a cell highlights it.</li>
 * </ul>
 * 
 * <p>
 * Users can right-click on a cell to quickly create a new task for that date or
...
public class SchedulingApp {
	...
	/**
	 * Constructs a {@code SchedulingApp} instance. Initializes the current date and
	 * a map for tasks, then calls {@link #initialize()} to set up the UI.
	 * 
	...
	/**
	 * Initializes the GUI components and sets up the main frame, panels, controls,
	 * and table. Also sets a timer to refresh the view every 5 seconds.
	 * 
	...
	/**
	 * Checks if a given row/column refers to a valid calendar cell (one that
	 * contains a LocalDate).
	 * 
	...
	private boolean isValidDateCell(int row, int col) {
		return row >= 0 && col >= 0 && calendarDates != null && row < calendarDates.length
				&& col < calendarDates[0].length && calendarDates[row][col] != null;
	}

	/**
	 * Sets up the right-click popup menu for calendar cells, providing options to
	 * create a task or show all tasks for the selected date.
	 */
	...
	/**
	 * Shows the popup menu at the given event location for the specified cell
	 * coordinates.
	 *
	...
	/**
	 * Updates the entire calendar view based on the selected year and month in the
	 * combo boxes.
	 * 
	...
	/**
	 * Renders the month view in the table for a given date. It calculates the
	 * LocalDate for each cell and displays the day of the month. Non-month days are
	 * grayed out.
	 * 
	...
	private void renderMonthView(LocalDate date) throws Exception {
		...
		DefaultTableCellRenderer headerRenderer = (DefaultTableCellRenderer) calendarTable.getTableHeader()
				.getDefaultRenderer();
	...
	/**
	 * Fetches tasks for the given month in a background thread. When done, updates
	 * cell rendering to highlight days with tasks.
	 * 
	...
	/**
	 * Updates the cell rendering after tasks are loaded. Highlights cells that have
	 * tasks, grays out days not in the current month, and applies hover and
	 * selection colors.
	 * 
	...
	/**
	 * Opens the {@link AddTaskDialog} for a specific date. The dialog allows the
	 * user to create a new task on the selected date.
	 * 
	...
	/**
	 * Shows all tasks for a specific date in a message dialog. If tasks are found,
	 * displays them in a list. Selecting a task and clicking OK opens a
	 * {@link ViewTaskDialog} for that task.
	 * 
	...
	private void showAllTasksForDate(LocalDate date) throws SQLException {
		List<Task> tasksForDate = tasks.stream().filter(task -> task.getDate().isEqual(date)).toList();
		...
		if (tasksForDate.isEmpty()) {
			...
			// If the user clicks OK and a task is selected, open the ViewTaskDialog with
			// the selected task
	...
	/**
	 * Returns an array of Integers representing the year range for selection. The
	 * range is 10 years before the current year to 9 years after.
	 *
