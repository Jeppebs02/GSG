...
/**
 * The {@code TaskCtrl} class manages the lifecycle of {@link Task} objects,
 * including creation, association with shifts, employees, and reports. It also
 * interacts with the database via {@link TaskDB} and {@link ReportDB} to
 * persist and retrieve task-related data.
 */
public class TaskCtrl {
	...
	/**
	 * Constructs a {@code TaskCtrl} instance and initializes a {@link TaskDB}
	 ...
	 * and user ID. The user is retrieved from the database, associated with the
	 * task, and the task is then saved to the database.
	 *
	 ...
	 * @param userID      the ID of the user (customer or guest) associated with the
	 *                    task.
	...
	/**
	 * Adds a new {@link Shift} to the current task using the specified start and
	 * expected end times. The shift is created using a {@link ShiftCtrl} instance
	 * and then associated with the current task.
	 * 
	...
	/**
	 * Assigns an {@link Employee} to the current shift of the current task by using
	 * the user ID. The employee is retrieved from the database and then assigned to
	 * the current shift through the {@link ShiftCtrl} instance. The shift is saved
	 * to the database afterwards.
	 * 
	 * @param userID the user ID associated with the employee to be assigned.
	 * @throws Exception if an error occurs during employee retrieval or shift
	 *                   saving.
	 */
	...
	/**
	 * Saves the current task to the database. This method creates a new
	 * {@link Report} with default or placeholder values, associates it with the
	 * task, and then saves both the task (using {@link TaskDB}) and the report
	 * (using {@link ReportDB}) to the database.
	 * 
	 * @throws Exception if an error occurs during the task or report saving
	 *                   processes.
	 */
	...
	/**
	 * Retrieves all tasks from the database that fall within the specified month
	 * and year.
	 * 
	 * @param year  the year to filter tasks by.
	 * @param month the name of the month (e.g. "JANUARY", "FEBRUARY") to filter
	 *              tasks by.
	...
	/**
	 * Retrieves a {@link Task} from the database by its unique task ID.
	 * 
	 * @param taskID the unique identifier of the task to be retrieved.
	 * @return the {@link Task} object with the specified ID, or null if not found.
	 * @throws Exception if a database access error or other issue occurs during
	 *                   retrieval.
	 */
